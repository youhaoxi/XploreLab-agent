import os
import copy
from typing import AsyncIterator, TypedDict, Union, Optional, Literal

from openai import AsyncOpenAI, AsyncStream
from openai.types.shared import ChatModel
from openai.types.chat import (
    ChatCompletionToolParam, ChatCompletionMessageParam,
    ChatCompletionChunk, ChatCompletionMessageToolCall, ChatCompletionMessage, 
)
...
class OpenAIChatCompletionParams(TypedDict):
    # NOTE: only for typing
    messages: list[ChatCompletionMessageParam]  # required
...
OpenAIChatCompletionParamsKeys = list(OpenAIChatCompletionParams.__annotations__.keys())
...
class SimplifiedAsyncOpenAI(AsyncOpenAI):
    ...
    def __init__(
        ...
        **kwargs: dict,
        ...
        # parse kwargs for client & default ChatCompletionParams
        default_config, openai_client_kwargs = {}, {}
        for k, v in kwargs.items():
            if k in OpenAIChatCompletionParamsKeys:
                default_config[k] = v
            else:
    ...
    async def query_one(self, **kwargs) -> str:
        ...
        chat_completion: ChatCompletion = await self.chat_completion(**kwargs)
