...
class PlannerAgent(Base):
    ...
    async def get_next_task(
        self, query=None, prev_task=None, prev_subtask_result=None, trace_id=None
    ) -> NextTaskResult:
    ...
    async def execute_planning(
        self,
        mode: Literal["planning", "update_planning"],
        question: str,
        background_info: str = "",
        previous_plan: str = "",
        task: str = "",
    ...
    async def create_plan(self, question: str, background_info: str = "", session_id: str = None) -> dict:
        ...
        session_record = {
            ...
            "mode": "planning",
            "request": {"question": question, "background_info": background_info},
            "response": result,
        }
    ...
    async def update_plan(self, task: str, task_results: str, session_id: str = None):
        ...
        result = await self.execute_planning(
            "update_planning", question, background_info, previous_plan, task, task_results, session_id=session_id
        )
        session_data = SessionManager.load_session(session_id)
        session_record = {
            "timestamp": datetime.now().isoformat(),
