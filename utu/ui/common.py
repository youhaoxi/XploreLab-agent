...
async def handle_raw_stream_events(event: ag.RawResponsesStreamEvent) -> Optional[Event]:
...
async def handle_orchestra_events(event: OrchestraStreamEvent) -> Optional[Event]:
    item = event.item
    if event.name == "plan":
        todo_str = []
...
async def handle_tool_call_output(event: ag.RunItemStreamEvent) -> Optional[Event]:
    item = event.item
...
async def handle_new_agent(event: ag.AgentUpdatedStreamEvent) -> Optional[Event]:
    if event.new_agent:
