...
def dump_exp_samples(config: EvalConfig, output_file: str, clear_records: bool = False):
    db = DBDataManager(config)
    samples = db.get_samples()
...
def main():
    ...
    match args.op:
        case "stat":
            get_stat(config)
        case "dump":
            dump_exp_samples(config, output_file=args.output_file, clear_records=args.clear_records)
